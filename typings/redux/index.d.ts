 ...
 * whether from UI events, network callbacks, or other sources such as WebSockets needs to eventually be dispatched as
 * actions
 *
 * Actions must have a `type` field that indicates the type of action being
 * performed. Types can be defined as constans and imported from another module. It's better to use strings for `type`
 * than Symbols because strings are serializable.
 *
 * Other than `type`, the structure of an action object is really up to you.
 * If you'are interested, check out Flex Standard Action for recommendations on how actions should be constructed.
 *
...
/**
 * A *reducer* (also called a *reducint function*) is a function that accepts an accumulation and a value and returns a
 * new accumulation and a value and returns a new accumulation. They are used to reduce a collection of values down to
 * a single value.
 *
 * Reducers are not unique to Redux-they are a fundamental concept in functional programming. Even most non-functional
 * languages, like JavaScript, have a built-in API for reducing. In JavaScript, it's `Array.prototype.reduce`.
 *
 * In Redux, the accumulated value is the state object, and the values being accumulated are actions. Reducers calculate
 * a new state given the previous state and on action. They must be *pure function*-functions that return the exact same
 * output for given outputs. They should also be free of side-effects. This is what enables exciting features like hot
 * hotreloading and time travel.
 *
